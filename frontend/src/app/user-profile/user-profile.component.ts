...
export class UserProfileComponent implements OnInit {
  ...
  loadFollowers() {
    ...
    this.profileService.getFollowers(this.user.username).subscribe({
      next: (followers) => {
        // followers are just objects with username, fetch full details
        const usernames = followers.map(f => f.username);
        if (usernames.length > 0) {
          this.profileService.fetchUsersByUsernames(usernames).subscribe({
            next: (fullProfiles) => {
              this.followers = fullProfiles;
            },
            error: (err) => {
              console.error('Error fetching full follower profiles:', err);
              this.followers = followers; // Fallback
            }
          });
        } else {
          this.followers = [];
  ...
  loadFollowing() {
    ...
    this.profileService.getFollowing(this.user.username).subscribe({
      next: (following) => {
        // following are just objects with username, fetch full details
        const usernames = following.map(f => f.username);
        if (usernames.length > 0) {
          this.profileService.fetchUsersByUsernames(usernames).subscribe({
            next: (fullProfiles) => {
              this.followingList = fullProfiles;
            },
            error: (err) => {
              console.error('Error fetching full following profiles:', err);
              this.followingList = following; // Fallback
            }
          });
        } else {
          this.followingList = [];
